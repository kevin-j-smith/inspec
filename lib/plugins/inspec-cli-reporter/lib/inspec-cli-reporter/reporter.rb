...
module InspecPlugins
  module CliReporter
    # The Cli Reporter reports run results to stdout
    class Reporter < Inspec.plugin(2, :reporter) # rubocop:disable Metrics/ClassLength, Metrics/LineLength
      ...
      def profile_summary # rubocop:disable Metrics/MethodLength
        failed = 0
        skipped = 0
        passed = 0
      ...
      def tests_summary # rubocop:disable Metrics/MethodLength
        total = 0
        failed = 0
        skipped = 0
        passed = 0
      ...
      def standard_controls_from_profile(profile)
        profile[:controls].reject { |c| anonymous_control?(c) }
      end

      def anonymous_controls_from_profile(profile)
        profile[:controls].select { |c| anonymous_control?(c) && !c[:results].nil? } # rubocop:disable Metrics/LineLength
      end
      ...
      class Control
        ...
        def title_for_report
          # if this is an anonymous control, just grab the
          #  resource title from any result entry
        ...
        def impact_string # rubocop:disable Metrics/MethodLength, Metrics/PerceivedComplexity, Metrics/LineLength
          if anonymous?
            nil
          elsif impact.nil?
            "unknown"
